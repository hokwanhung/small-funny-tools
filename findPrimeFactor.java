...
public class findPrimeNumber{
   ...
   static long maxPrimeFactors(long detectInput){
     ...
     while (detectInput % 2 == 0) { // If detectInput modulus(%) 2 has no remainder, the maximum prime no. is 2.
      ...
      // detectInput must be odd after removing all factor 2 in the detectInput. Thus, the even number are skipped.
      // Only odd integer would be iterated now.
      for (int i = 3; i <= Math.sqrt(detectInput); i += 2){ 
      // Extracting all possible 9dd factors from the smallest to the biggest until the square root of detectInput (WHY??).
         while (detectInput % i == 0){ 
         // If the detectInput can be divided by the current odd factor, try the current odd factor again.
            max_prime = i;
